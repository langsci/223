...
\ili{Lower Sepik-Ramu} & \ili{Lower Sepik} & yee & \ili{Yimas} & \citet{Foley1991}\\
...
\ili{West Papuan}\footnotemark{} & – & ayz & \ili{Maybrat} & \citet{Dol2007}\\
\lspbottomrule
\end{tabularx}
...
An important advantage of adopting Di Garbo's approach is that this makes the results for New Guinea directly comparable with Africa as for the selected criteria. In addition, since the first three criteria are the same as the ones used in the \textit{WALS} chapters by Corbett (\citealt{Corbett2013,Corbett2013a,Corbett2013b}), most of the results are comparable to a worldwide sample. In order to illustrate the distributions, maps were created using the Interactive Reference Tool of the \textit{World atlas of language structures} (\textit{WALS})\footnote{See http://www.eva.mpg.de/lingua/research/tool.php.} using ISO codes and coordinates from Glottolog.
...
%%please move \begin{table}[!htb] just above \begin{tabular
\begin{table}[!htb]
\begin{tabularx}{\textwidth}{XXXX}
\lsptoprule
...
\begin{table}[!htb]
\small
\begin{tabularx}{\textwidth}{X>{\itshape}X>{\itshape}X>{\itshape}Q>{\itshape}Q>{\itshape}Q}
\lsptoprule
